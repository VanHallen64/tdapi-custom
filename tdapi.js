...
var CI = require('./structures/ci');
var Relationship = require('./structures/relationship');
...
/**
 * Gets all configuration items that match the searching parameters.
 * @param {Number} appId  - The associated application ID to get attributes for.
 * @param {CISearch} searchParams - The searching parameters to use.
 * @returns {Promise<CI>}
 */
 TDAPI.prototype.getCIs = function (appId, searchParams) {
  return this.login()
    .then(bearerToken => {
      return request({
        method: 'POST',
        url: `${this.baseUrl}/${appId}/cmdb/search`,
        auth: { bearer: bearerToken },
        json: true,
        body: searchParams || {}
      });
    })
    .then(asset => new CI(this, asset))
    .catch(handleError);
};
 ...
 * @param {Number} appId  - The associated application ID to get attributes for.
 * @param {Number} id - The CI ID.
 * @returns {Promise<Array>}
 */
 TDAPI.prototype.getCIRelationships = function (appId, id) {
  return this.login()
    .then(bearerToken => {
      return request({
        method: 'GET',
        url: `${this.baseUrl}/${appId}/cmdb/${id}/relationships`,
        auth: { bearer: bearerToken },
        json: true,
      });
    })
    .then(relationship => new Relationship(this, relationship))
    .catch(handleError);
};
 ...
 TDAPI.prototype.editCI = function (id, ci) {
  return this.login()
    .then(bearerToken => {
      return request({
        method: 'POST',
        url: `${this.baseUrl}/${appId}/cmdb/${id}`,
        auth: { bearer: bearerToken },
        json: true,
        body: ci
      });
